...
public class StargateCloseVortexPacket {
    private final BlockPos dhdPos;
    private final int dimension;

    public StargateCloseVortexPacket(ExtendedPos dhdPos) {
        this.dhdPos = dhdPos.getPos();
        this.dimension = dhdPos.getDimension();
    }

    public StargateCloseVortexPacket(BlockPos dhdPos, int dimension) {
        this.dhdPos = dhdPos;
        this.dimension = dimension;
    }

    public static void encode(StargateCloseVortexPacket msg, PacketBuffer buf) {
        buf.writeBlockPos(msg.dhdPos);
        buf.writeInt(msg.dimension);
    }

    public static StargateCloseVortexPacket decode(PacketBuffer buf) {
        return new StargateCloseVortexPacket(buf.readBlockPos(), buf.readInt());
    }
    ...
    public static void handle(StargateCloseVortexPacket msg, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            if (te instanceof StargateControllerTileEntity) {
                ...
                StargateBaseTileEntity base = (StargateBaseTileEntity) world.getTileEntity(dhd.getLinkedStargate());
                if (base != null) {
                    base.disconnect();
                }
            }
