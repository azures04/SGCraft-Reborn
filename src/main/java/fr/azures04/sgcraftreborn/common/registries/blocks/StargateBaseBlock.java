...
public class StargateBaseBlock extends Block implements ILiquidContainer, IBucketPickupHandler {
        ...
        if (!world.isRemote) {
            ...
            if (base != null && base.getVortexState() != StargateVortexState.IDLE) {
                ...
                if (radius > 0) {
                    ...
                    Explosion.Mode explosionMode = Explosion.Mode.BREAK;
                    if (causesFire) {
                        explosionMode = Explosion.Mode.DESTROY;
                    }
                    world.createExplosion(null, pos.getX() + 0.5, pos.getY() + 2.5, pos.getZ() + 0.5, (float)radius, explosionMode);
                }
            }
        }
    ...
    public BlockRenderType getRenderType(BlockState state) {
        return state.get(INVISIBLE) ? BlockRenderType.ENTITYBLOCK_ANIMATED : BlockRenderType.MODEL;
    }
